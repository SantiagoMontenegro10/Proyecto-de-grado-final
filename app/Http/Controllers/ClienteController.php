    ...
    public function index()
    {
        ...
        $sql = DB::table('cliente')
            ...
            ->select(
                ...
                'cliente.telefono',
                'cliente.direccion',
                DB::raw('DATE(cliente.desde) as desde'),
                DB::raw('DATE(cliente.hasta) as hasta'),
                'cliente.DT',
                'cliente.DA',
                'cliente.DR',
                'cliente.foto',
   ...
   public function editarDatosCliente(Request $request)
{
    $request->validate([
        ...
        "idcliente" => "required",
        "huella_id" => "nullable|numeric"
    ]);

    $id = $request->idcliente;

    // Verificar duplicados (usuario, dni, correo o huella)
    $verifificarDuplicidad = DB::select("
    ...
    if ($verifificarDuplicidad[0]->total >= 1) {
        return back()->with("AVISO", "El usuario, DNI, correo o huella ya está en uso");
    }
...
public function descargarTarjeta()
{
    ...
    $filename = "clientes_gimnasio_tauro.csv";
    ...
    $callback = function () use ($clientes) {
        ...
        fputcsv($file, ['ID', 'Nombre', 'Correo', 'Teléfono', 'Cédula']);
